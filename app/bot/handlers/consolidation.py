...
async def start_consolidation(
    ...
    system_prompt = (
        f"Ты — дружелюбный преподаватель-репетитор. Твоя задача — помочь ученику закрепить тему «{lesson.title}». "
        f"Вот содержание урока: {lesson.content}...\n"
        "Задай вопрос по теме пройденого урока, чтобы проверить учащегося."
    )
